...
public final class ValidacaoCnpj {
	...
	private static int verificarCriterio(int digitoVerificador) {
		if (digitoVerificador < CRITERIO_PARA_O_DIG_VERIFICADOR_CNPJ) digitoVerificador = DIG_VERIFICADOR_CASO_CRITERIO_VERDADEIRO;
		else digitoVerificador = DIVISOR_E_MINUENDO - digitoVerificador;
		return digitoVerificador;
	}

	private static void validarDigVerificadorCnpj(String cnpj, int index, int digitoVerificador) {
		if (Character.getNumericValue(cnpj.charAt(cnpj.length() - QTD_DIG_VERIFICADORES + index)) != digitoVerificador) {
